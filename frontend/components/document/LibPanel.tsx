	...
	return (
		<div className={`${props.visibleState}`}>
			<div
				...
				onClick={() => setLibSectionBasic({ open: !libSectionBasic.open })}
			>
				<label> {'>'} Basic</label>
			</div>
			<div
				...
				style={
					libSectionBasic.open
						? { maxHeight: '100%' }
						: { overflow: 'hidden', maxHeight: '0px' }
				}
			>
				<div
			...
			<div
				className={styles.libSectionLabel}
				onClick={() => setLibSectionImport({ open: !libSectionImport.open })}
			>
				<label> {'>'} Import</label>
			</div>
			<div
