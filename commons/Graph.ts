...
class Graph {
	...
	public getNodesByNameAndStrictClass<T extends Node>(name: string, classType: Function): Node | null {
		var filtered = this.getNodesByName(name).filter(node => node.constructor.name === classType.name);
		console.assert(filtered.length < 2);
		if (filtered.length == 1) return filtered[0];
		return null;
	}
